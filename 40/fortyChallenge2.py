...
def main():
    ...
    field_to_filter_on = get_string_input("Please select field to filter on (n,p): ", ['n', 'p'])
    search_string = input("Enter a search string: ")

    fields_to_filter_on = list()
    if field_to_filter_on == 'n':
        fields_to_filter_on.append(FIRST_NAME)
        fields_to_filter_on.append(LAST_NAME)
    elif field_to_filter_on == 'p':
        fields_to_filter_on.append(POSITION)        
        
    filtered_persons = filter(persons, search_string, fields_to_filter_on)
