...
def sort_persons_by_salary(persons):
    sorted_persons = list()
    for person in persons:
        for (index, sorted_person) in enumerate(sorted_persons):
            if int(person[SALARY]) > int(sorted_person[SALARY]):
...
def main():
    ...
    sorted_persons = sort_persons_by_salary(persons)
    print_header(column_widths)
    print_persons(sorted_persons, column_widths)
