...
def main():
    choice = get_string_input("Filter on name(n) or position(p)? ", ['n', 'p'])
    search_string = get_string_to_search_for()
    if choice == 'n':
        filtered_persons = filter_on_name(search_string)
    elif choice == 'p':
        filtered_persons = filter_on_position(search_string)
    else:
