...
def input_and_convert_from_celsius_to_fahrenheit():
...
def input_and_convert_from_celsius_to_kelvin():
    c = get_positive_number_input("Please enter the temperature in Celsius: ")
    k = calculate_celsius_to_kelvin(c)
    print("The temperature in Kelvin is %.1f" % k)
...
def main():
    scale = input_scale()
    if scale == "C":
        input_and_convert_from_celsius_to_fahrenheit()
    elif scale == "K":
        input_and_convert_from_celsius_to_kelvin()
    else:
